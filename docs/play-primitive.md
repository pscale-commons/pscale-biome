...
- **World-general (2026-06-26).** `play` now serves its `world` arg: `read_S/T/I` (and the frame/voice/weave path) derive their block names from `world` (`<world>-spatial/-temporal/-identity`, default `upperton`), and the **cast is derived from the store** — a character joins a world by declaring a seat in its own shell (`shell.3`: `spot` + `contest` stats); the `SPOT`/`CONTEST` globals are gone, and both the resolve trigger and the frame's seat list follow the seated shells. Proven model-free on Upperton *and* a second world (`harbor` — a 2-character, 1d6 cast) through the same engine, re-verified over `POST /mcp`. Remaining world-pin: `window`/`scene` are global per store (one active world per store).
