...
> 📅 **Founding-era document (2026-05-09) — describes the original design question, not current state.** Since this was written the project advanced through the spark recut, genome v1→v5 (live & public on Railway since 2026-06-10), the mobius-3 collective (v007), the courier flows, and the interface. Still valuable as the founding vision — but for *current state* read the memory `project_biome_shape.md`, plus `docs/biome-shape.md`, `docs/handover.md`, `docs/interface-handover.md`. Verify specifics against current code/substrate before acting.
