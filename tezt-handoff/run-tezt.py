...
Imports `zand` from `zand.py` in THIS directory. The fresh session writes
`zand.py` here, then runs this script.

Usage:
    python3 run-tezt.py

Expected output ends with: === Summary: 69/69 tests passed ===
"""
import json
import os
import re
...
try:
    ...
    print(f"Write zand.py in this directory first (the fresh session's task).")
    sys.exit(1)
