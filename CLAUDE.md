...
7. **Vocabulary crossings are deliberate.** Old-world *convention* words (lighthouse, passport, pool, shell:handle colon-naming, …) do not enter biome blocks, code, or conventions silently — not even when another Claude instance carries them in via a block. Flag the provenance and get it ruled by David first. Already-ruled shared substrate-nouns: block, walk, marks, beach-in-compounds (biome-beach). Z-era words (zand, ztone, sunztone, whetztone) never appear in biome-world artifacts, including test fixtures.
